...
public class AppImple {
    ...
    public boolean registrarCliente(Cliente cliente) {
        ...
        return exito; // <--- IMPORTANTE: Devolvemos el resultado
    }
    ...
    // Método auxiliar para mantener coherencia con nombres usados en el controlador
    public void registrarPelicula(Pelicula pelicula) {
        this.cargarPelicula(pelicula);
    }

    public List<Pelicula> listarPeliculas(int orden) {
    ...
    public void listarResenias() {
        ...
        for (Reseña r : reseñas) {
            System.out.printf("ID: %d | Usuario: %s | Película ID: %d | Comentario: %s | Puntaje: %d\n", r.getID(),
                    r.getUsuario().getNombre(), r.getIDContenido(), r.getComentario(), r.getCalificacion());
        }
    }
    ...
    public List<Pelicula> buscarPeliculasPorTitulo(String titulo) {
        return this.peliculaDAO.buscarPorTitulo(titulo);
    }

    public boolean hayPeliculasCargadas() {
        return !this.peliculaDAO.listarTodas().isEmpty();
    }
    ...
    public boolean existeEmail(String email) {
        // Evitar dependencia directa en tiempo de ejecución con implementaciones
        // desincronizadas: recorremos la lista de usuarios y comparamos el email.
        try {
            for (Usuario u : this.usuarioDAO.listarTodos()) {
